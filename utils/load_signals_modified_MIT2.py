...
class PrepData():
    ...
    def preprocess(self, data_):
       ...
       # ictal_ovl_pt=0.50  #overlap 80%
        ictal_ovl_len = int(targetFrequency*ictal_ovl_pt*numts) # overlap length
        interictal_ovl_len = int(targetFrequency*1*numts)#overlap 50%

        def process_raw_data(mat_data):            
            print ('Loading data')
            if  ~ictal and (self.target in ['14','17','18','19','20','21']) and self.settings['dataset'] == 'FB' :
                    special_targets={'14':22,'17':24,'18':24,'19':24,'20':24,'21':23}
                    X_temp = []
                    y_temp = []
        ...
        #                print(data.shape)
                        ...
                        if ictal:
                            ...
                            while(window_len + (i)*interictal_ovl_len <= data.shape[0]):
                                    ...
                                    y_temp.append(y_value)
                                    i+=1
                    y_temp=np.array(y_temp[0:i*special_targets[self.target]],dtype='float32')
                    X_temp=np.array(X_temp[0:i*special_targets[self.target]],dtype='float32')
                  
                    X=X_temp.reshape(-1,i,window_len,channels)
                    y=y_temp.reshape(-1,i)
                            
                            
                             
                                   
            elif ~ictal and (self.target in ['20','9','10','23']) and self.settings['dataset'] == 'CHBMIT' : #data with different dimentions for each interictal
                special_targets={'9':12,'20':20,'23':3,'10':11}
    ...
    #                    y=np.vstack((y,y_temp[np.newaxis,:]))
                    ...
                    X.append(X_temp)
                    y.append(y_temp) 
                v=np.concatenate(X, axis=0).shape[0]
                k=X_temp.shape[0]*len(X)
                if k<v:
